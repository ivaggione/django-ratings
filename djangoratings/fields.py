...
class RatingManager(object):
    ...
    def add(self, score, user, ip_address, cookies={}, commit=True):
        ...
        try:
            ...
            raise e.InvalidRating("%s is not a valid choice for %s" % (score, self.field.name))

        delete = (score == 0)
        if delete and not self.field.allow_delete:
            raise e.CannotDeleteVote("you are not allowed to delete votes for %s" % (self.field.name,))
        ...
        if is_anonymous and not self.field.allow_anonymous:
            raise e.AuthRequired("user must be a user, not '%r'" % (user,))
        ...
        try:
            ...
            if delete:
                raise e.CannotDeleteVote("attempt to find and delete your vote for %s is failed" % (self.field.name,))
        ...
        if has_changed:
            if not delete:
                self.score += rating.score
            if commit:
                self.instance.save(update_fields=['rating_score', 'rating_votes'])
